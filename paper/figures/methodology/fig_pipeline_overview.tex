...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        stage/.style={
            ...
            fill=blue!8, minimum width=4.2cm, minimum height=0.9cm,
            text centered, font=\small\bfseries
        },
        substage/.style={
            rectangle, rounded corners=2pt, draw=black!50,
            fill=white, minimum width=3.8cm, minimum height=0.55cm,
    ...
    % ── Stage 1: Extraction and Recognition ──
    \node[stage, below=0.5cm of input] (extraction) {Stage 1: Extraction \& Recognition};
    ...
    \node[substage, below=0.06cm of charfilter]
        (layout) {Layout analysis};
    \node[substage, below=0.06cm of layout]
        (chatocr) {CHAT OCR + matching};

    % ── Stage 2: Preprocessing ──
    \node[stage, below=0.6cm of chatocr] (preproc) {Stage 2: Preprocessing};
    \node[substage, below=0.1cm of preproc.south west, anchor=north west, xshift=0.2cm]
        (skew) {Rotation correction (LSD)};
    \node[substage, below=0.06cm of skew]
        (inkfilt) {Ink filter};
    \node[substage, below=0.06cm of inkfilt]
        (vect) {Vectorisation (GASS)};

    % ── Stage 3: Matching and Clustering ──
    \node[stage, below=0.6cm of vect] (cluster) {Stage 3: Matching \& Clustering};
    \node[substage, below=0.1cm of cluster.south west, anchor=north west, xshift=0.2cm]
        (hogcomp) {HOG computation};
    \node[substage, below=0.06cm of hogcomp]
    ...
    % ── Stage 4: Refinement ──
    \node[stage, below=0.6cm of leiden] (refine) {Stage 4: Refinement \& Glossary};
    ...
    \node[substage, below=0.06cm of hausdorff]
        (labelsplit) {Label-based splitting};
    \node[substage, below=0.06cm of labelsplit]
        (glossary) {Glossary construction};

    % ── Output ──
    \node[io, below=0.5cm of glossary] (output) {Vectorised character glossary};

    % ── Arrows ──
    \draw[arrow] (input) -- (extraction);
    \draw[arrow] (chatocr.south) -- ++(0,-0.2) -| (preproc);
    \draw[arrow] (vect.south) -- ++(0,-0.2) -| (cluster);
    \draw[arrow] (leiden.south) -- ++(0,-0.2) -| (refine);
    \draw[arrow] (glossary.south) -- ++(0,-0.2) -| (output);
    ...
    Stages are annotated with their computational scope (per-page, per-patch, or global) and primary hardware usage.}
