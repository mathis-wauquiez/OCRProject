...
The baseline pipeline described in Section~\ref{sec:methodology} already produces a functional character glossary.
We now outline four categories of potential extensions that address specific limitations identified during development: alternative clustering strategies (Sect.~\ref{sec:proposed-clustering}), principled refinement (Sect.~\ref{sec:proposed-refinement}), registration methods for intra-cluster template computation (Sect.~\ref{sec:proposed-registration}), and deformation-invariant features (Sect.~\ref{sec:proposed-features}).
These directions are presented to guide future evaluation.
...
Three alternatives operating on the continuous NLFA matrix may improve on this.
...
This subsumes the two baseline refinement stages: pairwise potentials capture visual similarity (Stage~1), unary potentials encode the OCR-guided PCA compatibility test (Stage~2), and global energy minimisation jointly optimises assignment.
...
An alternative to the two sequential refinement stages is a single iterative algorithm.
