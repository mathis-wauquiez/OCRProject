...
\paragraph{Stage~2: OCR-guided PCA rematching.}
Small clusters ($\le 3$ members) are candidates for merging.
For each, the dominant OCR label selects target clusters: all large clusters ($\ge 10$ members) whose own dominant label matches.
This OCR-guided shortlist replaces blind nearest-neighbour search in feature space, focusing the expensive registration step on semantically plausible candidates.
Each candidate is then validated via image-space PCA compatibility: the query patch is registered against the candidate's most central member using IC alignment, projected into the candidate's PCA space ($d=5$ components), and accepted only if $\max_j |z_j| < 3.0$.
Among compatible candidates, the one with the lowest aggregate z-score receives the merge.
