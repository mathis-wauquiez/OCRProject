...
\begin{enumerate}
    ...
    \item \textbf{Mask expansion.}  The basin mask is defined at a lower threshold $\tau_\text{mask} = 0.3$, allowing character regions to expand into areas with moderate CRAFT scores.  This is particularly important for composite CJK characters where sub-parts (radicals) may receive lower text scores.
...
\begin{equation}
    \theta = \frac{1}{2}\arg\!\left(\sum_i w_i \, e^{2\mathrm{i}\alpha_i}\right),
    \label{eq:skew}
\end{equation}
