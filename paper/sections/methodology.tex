...
%  a. CRAFT character detection
%  b. dubious character extraction based on this detection and binary connected components
%     | see the slides
%  c. determine the columns, sub-columns and reading order via projection-based layout analysis
% 2. OCR
%   We use a Kraken-based model trained on similar historical Chinese documents, operating at the sub-column level. Baseline is extracted by taking the median of the x-barycenters of the characters
...
%   When no characters can be matched, we assign a token representing an unknown character (e.g. $\square$).
...
Communities are detected using the Leiden algorithm~\cite{traag2019louvain} with the Constant Potts Model (CPM) quality function, which minimises the correlation clustering disagreement objective:
\begin{equation}
    \mathcal{Q}_\gamma = \sum_{c} \left[e_c - \gamma \binom{n_c}{2}\right],
\end{equation}
where $e_c$ is the total edge weight within community $c$, $n_c$ its size, and $\gamma$ is the resolution parameter.
We sweep over the NFA threshold $\varepsilon$ and the resolution parameter $\gamma$ to find the partition maximising ARI with respect to OCR labels (see Section~\ref{sec:sensitivity}).
...
Registration is performed via multiscale inverse compositional alignment, which compensates for positional and scale differences between character patches.
