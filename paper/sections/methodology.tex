...
Filters discard components outside the expected size range ($30 \times 30$ to $150 \times 150$\,px) or with insufficient filled area ($<200$\,px$^2$).
...
Each character is described by a HOG descriptor~\cite{dalal2005histograms} computed on a rendered SVG image ($22 \times 22$\,px cells, 256\,DPI).
...
Large clusters ($\ge 5$ members) are inspected using pairwise Hausdorff distances~\cite{rony2025hausdorff} on binary images.
...
This step enforces label consistency within clusters without relying on image-space registration.
