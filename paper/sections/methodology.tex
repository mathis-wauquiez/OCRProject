...
Our goal is to produce, from a collection of scanned historical document pages, a vectorised glossary of the distinct character forms they contain—together with the ability to reprint any page from that glossary alone.
The key observation behind the pipeline is that a printed book contains many copies of each character, and that the printing defects affecting each copy are essentially random: ink may spread, paper may be locally damaged, or the woodblock may have worn unevenly, but these artefacts vary from one occurrence to the next.
By comparing all instances of the same character, the \emph{cleanest} copy—the one closest to the intended form—can be identified as the one that best matches all the others, since it is statistically unlikely for the same defect to appear in every copy.

Turning this observation into a working system requires solving four successive problems.
First, individual characters must be located on each page and tentatively identified (\S\ref{sec:extraction}).
The resulting raster patches, however, differ in resolution, skew, and noise level, so a normalisation step converts them into a representation suitable for geometric comparison (\S\ref{sec:preprocessing}).
With comparable representations in hand, a statistical framework decides which characters depict the same glyph and groups them accordingly (\S\ref{sec:acontrario}).
Finally, a refinement stage corrects errors in that grouping, selects the cleanest representative for each character type, and reconstructs pages from the recovered models (\S\ref{sec:refinement}).
...
Before characters can be compared, they must be located.
This stage takes a scanned page image and returns, for each detected character, a binary image, a bounding box, and an OCR-predicted label.

% ── (a) CRAFT-based character detection ──
\paragraph{Character detection.}
Character regions are detected with CRAFT~\cite{baek2019character} (\textit{Character Region Awareness for Text Detection}), a fully convolutional network that produces per-pixel text score maps $S_\text{text}(x, y) \in [0,1]$.
Unlike word-level detectors, CRAFT localises individual characters, which is essential for CJK scripts where each character is a self-contained unit without inter-character ligatures.
We use pre-trained weights without fine-tuning, at magnification ratio~$5.0$ (canvas size 1280\,px).

Individual character regions are extracted via dual-threshold watershed: pixels with $S_\text{text} \ge \tau_\text{text} = 0.6$ serve as seeds, and basins expand until $S_\text{text}$ drops below $\tau_\text{mask} = 0.3$.
...
CRAFT produces soft, fuzzy score maps; we need crisp binary masks.
The page is therefore binarised in parallel with Otsu's method, yielding sharp connected components.
These components must then be matched to CRAFT detections: each binary component is assigned to the CRAFT region whose elliptic distance, measured via negative Mahalanobis distance from the region's inertia tensor, is smallest.
...
The result is a set of character candidates that carry both a binary image and a precise bounding box.
...
Once characters are detected, they must be ordered.
...
Characters are labelled by a Kraken-based~\cite{kiessling2019kraken} model trained on similar historical Chinese documents, operating at the sub-column level.
...
More fundamentally, pixel-level comparison conflates true shape differences with irrelevant imaging artefacts.
...
This stage achieves that by correcting rotation, cleaning the binary mask, and converting the result into a smooth vector outline.
...
The corrected binary patches are then cleaned by area opening and area closing, removing foreground specks below 5\,px and background holes below 10\,px.
The result is a crisp binary image ready for vectorisation.

% ── (c) Vectorisation via affine scale-space ──
\paragraph{Vectorisation via affine scale-space.}
The cleaned patches are vectorised using affine scale-space smoothing~\cite{alvarez1993axioms,ciomaga2017curvature}, a geometric PDE that evolves each level line according to its curvature:
\begin{equation}
...
This property is particularly relevant for woodblock-printed documents, where the printing pressure, paper curvature, and ink spread introduce local affine distortions that differ from one copy to the next.
Ordinary Gaussian smoothing would reduce noise but also blur sharp stroke endings; affine scale-space smoothing instead removes small-scale irregularities while preserving the large-scale calligraphic structure.
...
With all characters represented as comparable vector outlines, the central question becomes:
which of them depict the same glyph?
This stage answers it in three steps—describing each character with a shape descriptor, scoring pairwise similarity within a principled statistical framework, and partitioning the resulting similarity graph into clusters.

% ── (a) HOG descriptors ──
\paragraph{HOG descriptors.}
Each character outline is described by a Histogram of Oriented Gradients (HOG) descriptor~\cite{dalal2005histograms}.
...
Pairs of HOG descriptors $\mathbf{h}^A, \mathbf{h}^B$ (each with $K$ cells of $B$ bins) are compared using the Circular Earth Mover's Distance (CEMD) per cell.
...
Beyond deciding which characters match, the \textit{a contrario} framework provides a natural criterion for selecting the best representative of each character type.
Each printed occurrence of a character can be thought of as the ideal glyph corrupted by random defects—ink spread, paper damage, uneven woodblock wear.
Because these defects are essentially random, the probability that the same artefact appears in every copy is negligible.
The cleanest copy, the one closest to the ideal form, will therefore be meaningfully similar to the most other copies: its NFA scores will be low (i.e.\ its NLFA scores high) against a large number of partners, giving it the highest degree in the similarity graph.
Copies with severe defects, by contrast, will fail the NFA test against many partners, because their distortions push them away from the shared ideal.
This means that, once clustering is done, the most central node in each cluster—the one connected to the most neighbours—is also the one most likely to be defect-free.
We exploit this property directly in glossary construction (\S\ref{sec:refinement}).
...
\begin{equation}
    ...
    \label{eq:reciprocal}
\end{equation}
The reciprocal condition is important: similarity in the NFA sense is directional, because each character has its own null distribution.
Character~$A$ may find~$B$ surprisingly close (relative to $A$'s own distribution), while $B$ does not find~$A$ surprising (relative to $B$'s distribution).
Requiring both directions ensures that only mutually confirmed similarities produce edges, filtering out spurious one-sided matches.
...
To catch such cases, clusters with five or more members are re-examined using pairwise Hausdorff distances~\cite{rony2025hausdorff} computed on registered binary images.
...
For each OCR-predicted character, the cluster containing the most occurrences is identified, and its most central member—by degree centrality in the NFA similarity graph—is chosen as the representative.
...
For each character detection on a page, the glossary representative of its cluster is looked up and its SVG outline is rendered at the corresponding bounding-box position and scale.
