...
We detect character regions with CRAFT~\cite{baek2019character} (\textit{Character Region Awareness for Text Detection}), a fully convolutional network that produces per-pixel text score maps $S_\text{text}(x, y) \in [0,1]$.
Unlike word-level detectors, CRAFT localises individual characters—essential for CJK scripts, where each character is a self-contained unit without inter-character ligatures.
We use pre-trained weights without fine-tuning, at magnification ratio~$5.0$ (canvas size 1280\,px).

From these score maps, individual regions are extracted via dual-threshold watershed: pixels with $S_\text{text} \ge \tau_\text{text} = 0.6$ serve as seeds, and basins expand until $S_\text{text}$ drops below $\tau_\text{mask} = 0.3$.
...
CRAFT produces soft, fuzzy score maps; what we need are crisp binary masks.
