...
Achieving this requires solving four successive problems.
...
Finally, a refinement stage corrects errors in that grouping, selects a representative for each character type, and reconstructs pages from the recovered models (\S\ref{sec:refinement}).
...
In parallel, the page is binarised with Otsu's method.
The resulting binary connected components must then be associated with CRAFT detections: each component is assigned to the CRAFT region whose elliptic distance, measured via negative Mahalanobis distance from the region's inertia tensor, is smallest.
This yields character candidates that carry both a binary image and a precise bounding box.
...
This stage normalises them into resolution-independent SVG outlines that faithfully preserve the original calligraphy while enabling robust geometric comparison downstream.
...
Document skew is estimated with LSD~\cite{von2010lsd} (\textit{Line Segment Detector}).
...
The $1/3$ exponent makes the evolution affine-invariant: the smoothed shape does not depend on the viewing angle, a property essential for comparing characters printed under varying conditions.
...
This stage answers it in three steps—describing each character, scoring pairwise similarity within a principled statistical framework, and partitioning the resulting similarity graph.
...
Raw dissimilarities are not directly comparable across characters—a rare, intricate glyph will have a different baseline distance distribution than a frequent, simple one.
We therefore assess similarity through an \textit{a contrario} framework~\cite{desolneux2000meaningful}, which provides a statistically grounded, per-character matching threshold.
...
\begin{equation}
    ...
    \label{eq:reciprocal}
\end{equation}
The reciprocal condition ensures that similarity is mutual, discarding asymmetric matches.
...
Clusters with five or more members are inspected using pairwise Hausdorff distances~\cite{rony2025hausdorff} computed on registered binary images.
...
The result is a faithful vector reproduction of the original page layout, assembled entirely from the recovered character models.
