...
\begin{table}[t]
    ...
    \begin{tabular}{@{}llp{5.5cm}@{}}
        ...
        A11 & Refinement method & None, 3-stage, K-medoids, MRF, HDBSCAN \\
        A12 & MRF weight $\beta$ & $\{0.1, 0.5, 1, 2, 5\} / \text{med.\ NLFA}$ \\
        A13 & Stage ablation & Full, $-$S1, $-$S2, $-$S3, none \\
        \bottomrule
    \end{tabular}
\end{table}
...
The pipeline is run on subsets of increasing size ($n \in \{500, 1\text{k}, 2\text{k}, 5\text{k}, 10\text{k}, 15\text{k}\}$) to verify complexity bounds: NFA computation $O(N^2)$, HDBSCAN $O(N k + N \log N)$, MST alignment $O(mk \cdot HW)$ per cluster, MRF-BP $O(|\mathcal{E}| \cdot K \cdot T)$.
