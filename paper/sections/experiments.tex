...
The pipeline performs a grid sweep over epsilon thresholds ($\varepsilon \in \{1.5{\times}10^{-4}, 2{\times}10^{-4}, 2.5{\times}10^{-4}, 3.8{\times}10^{-4}, 10^{-3}\}$), Leiden resolution ($\gamma \in \{1.0, 1.25, 1.5, 1.75, 2.0\}$), and partition types (RBConfiguration, CPM).
