...
def compute_epe(T_est, T_gt, H, W, device):
    ...
    Returns (H, W) tensor (squeezed from batch dim).
    """
    ...
    # transform_points returns (B, H, W) — squeeze B=1
    xx_gt, yy_gt = T_gt.transform_points(xx, yy)
    xx_est, yy_est = T_est.transform_points(xx, yy)
    return torch.sqrt((xx_gt[0] - xx_est[0])**2 + (yy_gt[0] - yy_est[0])**2)
...
def create_single_scale_ic(device, C=1):
    ...
    return InverseCompositional(
        ...
        delta=5, epsilon=1e-3, max_iter=200,
        lambda_init=80.0, lambda_min=5.0, lambda_decay=0.9
    )
...
def create_multiscale_ic(device, C=1, min_size=32):
    ...
    pyramid = GaussianPyramid(eta=0.5, sigma_0=0.6, ksize_factor=8, min_size=min_size)
    return MultiscaleIC(
        singleScaleIC=ic,
        gaussianPyramid=pyramid,
        first_scale=0, grayscale=True
    )
...
def test_displacement_robustness():
    ...
    for tx in translations:
        T_gt = PlanarTransform('homography',
                               params=[0.05, 0.02, float(tx), -0.02, 0.04,
                                       float(tx)*0.8, 0.0001, -0.0001],
                               device=device)
        I1 = T_gt.warp(I2)
        I1_gray = 0.299 * I1[:, 0:1] + 0.587 * I1[:, 1:2] + 0.114 * I1[:, 2:3]
        ...
        ss_epes.append(epe_ss); ms_epes.append(epe_ms)
...
def visualize_results(T_ss, T_ms, T_gt, epe_ss, epe_ms, I1, I2, pyr):
    ...
    # EPE maps — epe_ss, epe_ms are (H, W)
    ...
    for i, level in enumerate(pyr):
        ...
        if x_offset + w <= W:
            lev = level.expand(-1, 3, -1, -1) if level.shape[1] == 1 else level
            composite[0, :, :h, x_offset:x_offset+w] = lev[0]
            x_offset += w + 2
    ax9.imshow(composite[0].permute(1, 2, 0).cpu().clamp(0, 1))
    ax9.set_title(f'Gaussian Pyramid ({len(pyr)} scales)'); ax9.axis('off')

    # Matrices — matrix is (1, 3, 3), squeeze for display
    ax10 = fig.add_subplot(3, 4, 10); ax10.axis('off')
    matrix_text = (f"Ground Truth:\n{T_gt.matrix[0].cpu().numpy()}\n\n"
                   f"Multiscale:\n{T_ms.matrix[0].cpu().numpy()}\n\n"
                   f"Single-scale:\n{T_ss.matrix[0].cpu().numpy()}")
...
def visualize_displacement_test(translations, ss_epes, ms_epes):
    ...
    ax.semilogy(translations, ss_epes, 'r-o', lw=2, ms=8, label='Single-scale')
    ax.semilogy(translations, ms_epes, 'b-s', lw=2, ms=8, label='Multiscale')
