...
def setup_pipeline(config_name="extraction_pipeline"):
    ...
    # Use absolute path with initialize_config_dir
    config_dir = str(PROJECT_ROOT / "confs")
    
    if not Path(config_dir).exists():
        raise FileNotFoundError(f"Config directory not found: {config_dir}")
    
    initialize_config_dir(config_dir=config_dir, job_name="extraction", version_base=None)
...
def main():
    parser = argparse.ArgumentParser(description="Process images through OCR pipeline")
    parser.add_argument('--image-folder', type=Path, default='data/datasets/book1',
                        help='Input image folder (default: data/datasets/book1)')
    parser.add_argument('--save-folder', type=Path, default='tests_2/book1-complete',
                        help='Output folder (default: tests_2/book1-complete)')
    ...
    image_folder = args.image_folder if args.image_folder.is_absolute() else PROJECT_ROOT / args.image_folder
    save_folder = args.save_folder if args.save_folder.is_absolute() else PROJECT_ROOT / args.save_folder
    
    # Get files
    if not image_folder.exists():
        parser.error(f"Image folder not found: {image_folder}")
    
    files = [f.name for f in image_folder.iterdir() if f.is_file()]
    if not files:
    ...
    if args.workers > 1:
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = [executor.submit(process_file, f, image_folder, 
                                      save_folder, pipeline) for f in files]
            results = [future.result() for future in tqdm(futures, desc="Processing")]
    else:
        results = [process_file(f, image_folder, save_folder, pipeline) 
